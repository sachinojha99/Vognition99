 ...
 *
 */
public class Vognition extends CordovaPlugin {
	    public void initialize(CordovaInterface cordova, CordovaWebView webView) {
        super.initialize(cordova, webView);
    }
	public static final String HOME_PROFILE_ID = "HPDL_ID";
    ...
    public UserCredential createUser() {
        UserCredential newCredentials = null;
        VognitionResponse response = performRestfulCommand("/apiv1/user", new VognitionContext(), ServerOperation.POST);
        if (response.wasSuccessful()) {
            newCredentials = new UserCredential();
            setConKey(response.getValue("conkey"));
            setSecretConKey(response.getValue("consecret"));
            newCredentials.conKey = getConKey();
            newCredentials.conKeySecret = getSecretConKey();
        }
        else informListeners(RequestStatus.RequestState.ERROR, response.textToUser);
    ...
    public String createHomeProfile() {
        ...
        if (!(getConKey()==null) && !(getSecretConKey() == null)) {
            VognitionContext context = new VognitionContext();
                ...
                if (response.wasSuccessful()) {
                    homeProfileID = response.getValue(HOME_PROFILE_ID);
                }
        }
        else {
    ...
    public boolean updateHomeProfile(String HPDL_ID, String homeProfile) {
        ...
        if (homeProfile.hashCode() != previous_home_profile_hash) {
            context.put(HOME_PROFILE_ID, HPDL_ID);
            persistentContext.put(HOME_PROFILE_ID, HPDL_ID);
                ...
                uploadThread = new Thread(new Runnable() {
                    public void run() {
                        ...
                        try {
                            ...
                            String HPDL_ID = (String) currentVContext.get(HOME_PROFILE_ID);
